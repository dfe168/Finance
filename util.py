...
def csv_to_dataset(csv_path):
    ...
    ohlcv_histories_normalised = np.array([data_normalised[i: i+history_points].copy() for i in range(len(data_normalised) - history_points)]) #normalize array van alle data (0 tot 1)

    next_day_open_values_normalised = np.array([data_normalised[:, 0][i + history_points].copy() for i in range(len(data_normalised) - history_points)]) # 0 = 0th element (open price)
    next_day_open_values_normalised = np.expand_dims(next_day_open_values_normalised, -1)

    # to get the data working with Keras, make the y array 2-dimensional
    ...
    def calc_ema(values, time_period):
        ...
        ema_values = [sma]
        k = 2 / (1 + time_period) #Exponential Moving Average formule
